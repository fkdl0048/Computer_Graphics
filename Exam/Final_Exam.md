...
가장 먼저 정규화된 뷰 볼륨을 만들기 위해 삼각함수로 계산한 행렬을 추출했다.

현재 형태는 정규화된 볼륨이지만 피라미드의 윗 부분형태이기 때문에 이를 평행 뷰 볼륨으로 변환하기 위해 Unhinging transformation을 수행한다.
...
원근감은 눈의 깊이 지각(depth perception)과 관련이 있습니다. 깊이 지각은 물체 간의 상대적인 거리를 인지하는 능력을 말하며, 이는 눈의 양쪽에 위치한 두 눈으로 인해 가능해집니다. 이러한 깊이 지각을 통해 우리는 물체의 크기, 위치, 및 거리에 대한 정보를 얻을 수 있습니다.

두 눈은 서로 약간의 간격이 있기 때문에, 각각의 눈에서 보이는 이미지는 조금씩 다릅니다. 이를 이용하여 뇌는 물체까지의 거리를 계산하고 깊이 지각을 형성합니다. 이것이 스테레오 시각(stereoscopic vision)의 원리입니다.

가까운 물체는 두 눈으로 볼 때 각 눈에서 관찰되는 이미지 간의 차이가 큽니다. 이 차이를 통해 뇌는 물체가 가까이 있다는 정보를 얻고, 그 결과로 물체를 크게 인식합니다. 반면에 먼 물체는 두 눈에서 볼 때 이미지 간의 차이가 작기 때문에 뇌는 물체가 먼 곳에 있음을 인식하고, 물체를 작게 인식합니다.

이러한 눈의 스테레오 시각을 통한 깊이 지각은 원근감을 형성하는 주요 원리 중 하나입니다. 눈의 원근 시각은 우리가 일상적으로 물체의 크기와 거리를 올바르게 인식할 수 있게 해줍니다.
