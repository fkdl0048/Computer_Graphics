...
void drawRobotArm1()
{
	...
	// BASE
	CTM = Translate(0, -0.5, 0) * RotateY(g_time * 30);
	M = Scale(0.3, 0.2, 0.2);
	...
	M = Translate(0, 0.05, 0) * Scale(0.2, 0.15, 0.1);
	glUniformMatrix4fv(uMat, 1, true, CTM * M);
	cube.Draw(program);
}
...
void drawRobotArm2()
{
	...
	stack.push_back(CTM);
	// LOWER ARM 1
	CTM = CTM * Translate(-0.2, 0, 0) * RotateZ(ang2);
	M = Translate(0, 0.2, 0) * Scale(0.1, 0.4, 0.1);
	glUniformMatrix4fv(uMat, 1, true, CTM * M);
	cube.Draw(program);
	...
	stack.push_back(CTM);
	// LOWER ARM 2
	CTM = CTM * Translate(0.2, 0, 0) * RotateZ(ang3);
	M = Translate(0, 0.2, 0) * Scale(0.1, 0.4, 0.1);
	glUniformMatrix4fv(uMat, 1, true, CTM * M);
	cube.Draw(program);
}
