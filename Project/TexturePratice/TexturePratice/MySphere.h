...
GLuint MySphere::Init(int la_slice, int lo_slice, vec4 color)
{
	...
	for(int i=0; i<la_slice; i++)
	{
		...
		for(int j=0; j<lo_slice; j++)
		{
			...
			vec2 ta = vec2((float)j/lo_slice, (float)i/lo_slice);
			vec2 tb = vec2((float)j / lo_slice, (float)(i + 1) / lo_slice);
			vec2 tc = vec2((float)(j + 1) / lo_slice, (float)(i + 1) / lo_slice);;
			vec2 td = vec2((float)(j + 1) / lo_slice, (float)i / lo_slice);;

		
			if(i!=lo_slice-1)
			{
				Vertices[cur].position = a;	Vertices[cur].color = color; Vertices[cur].normal = a; Vertices[cur].texCoord = ta; cur++;
				Vertices[cur].position = b;	Vertices[cur].color = color; Vertices[cur].normal = b; Vertices[cur].texCoord = tc; cur ++;
				Vertices[cur].position = c;	Vertices[cur].color = color; Vertices[cur].normal = c; Vertices[cur].texCoord = td; cur ++;
			}
			if(i!=0)
			{
