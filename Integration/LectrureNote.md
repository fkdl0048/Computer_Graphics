...
실제로 물체가 멀리있으면 작게 보이고 가까이 있으면 크게 보이는 것을 표현

#### 왜 원근감을 느끼는 가?

초점으로 빛이 모인다. (눈으로 물체를 관찰할 때 빛이 모이는 곳이 초점)

- 핀홀 카메라
  - 빛이 모이는 점을 만들어서 상이 거꾸로 보이게 만듬

눈도 마찬가지로 눈에 들어올 때 상이 맺히는 크기가 빛이 들어오는 각도에 따라 달라지기 때문에 원근감을 느낌

컴퓨터 모델은 굳기 뒤에 상을 맺히게 하여 뒤집을 필요 없이 앞에 두어 물체를 그대로 담음
...
쉐이더 자체가 영화관련 사업에서 발전되어 왔기에 대부분 영화관련 용어를 사용함

실세계를 흉내내기 보다 실제 영화에서 사용되는 감도과 조명, 반사판등의 역할로 이뤄짐

### Illumination Model

실제를 보고 직관적으로 바꿀 수 있도록 만든 모델

Lighting, Shading 모델로도 불림
...
- Area Light
  ...
  - (형광등)
  - 그림자의 경계면이 뭉개짐
  - 비용이 많이 들기 때문에 잘 사용하지 않음

조명모델을 Illumination Model이라고 하는데 그 중에 Phong Model이 있다. (유용)
...
표면이 거칠다면 빛이 반사되는 방향이 다양하게 반사된다. (벽)

매끄럽다면, 빛이 반사되는 방향이 한 방향으로 집중된다. (정반사, 거울)
