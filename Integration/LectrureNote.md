...
이미지의 한 점이 3d공간의 한 점에 매핑되는 것

x,y 좌표를 사용하지 않고 u,v(s, t) 좌표를 사용한다.

항상 최대값은 1이다. (따라서 중앙은 0.5)
...
좌표를 넘겨줄 때, 텍스쳐 좌표를 넘겨준다. (u, v)

fragment shader에서 텍스쳐 좌표를 받아서 텍스쳐 좌표에 맞는 픽셀을 찾아서 색을 결정한다.

- 0에서 1범위를 벗어난다면?
  - 소수점만 사용한다 (repeat)
  - 1보다 크면 1인 것 처럼, 0보다 작으면 0인 것 처럼 (clamp)

유니폼 메모리로 텍스처가 넘어간다.

### texture functions

반대로 현재 좌표에 필요한 텍스처 좌표를 찾는 것
